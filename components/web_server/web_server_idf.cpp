...
class IDFBackend : public IWebServer {
 public:
  ...
  // -------------------------------------------------------------------------
  // send_event — safe to call from any task/thread
  // -------------------------------------------------------------------------
  void send_event(const char *data, const char *event_type) override {
    ...
    for (auto it = sse_clients_.begin(); it != sse_clients_.end();) {
      // FIX: Use raw send() with MSG_DONTWAIT instead of httpd_socket_send().
      //      httpd_socket_send() is NOT safe to call from outside the httpd
      //      worker task that owns the connection → race conditions / crashes.
 ...
 private:
  ...
  // FIX: Disable Nagle's algorithm so SSE frames are flushed immediately
  //      instead of being held by the TCP stack → eliminates most "random
  //      delay" complaints.
  ...
  // ── SSE handler ────────────────────────────────────────────────────────────
  // FIX: Old version blocked its httpd worker thread in while(true)+vTaskDelay.
  //      With the default pool size, even 2 SSE clients starved all other
  //      requests (page load, /api/state, etc.) causing the "sometimes works,
  //      sometimes huge delay" symptom.
  //
  //      New design:
  //        1. Send headers + full_state snapshot using chunked transfer.
  //        2. Register the raw socket fd in sse_clients_.
  //        3. Return ESP_OK immediately — releases the httpd worker thread.
  //        4. A per-client FreeRTOS task sends periodic pings over the raw fd
  //           and removes the client when the socket dies.
  //        5. Future state pushes arrive via send_event() → raw send().
  // ──────────────────────────────────────────────────────────────────────────
  static esp_err_t h_events(httpd_req_t *req) {
    ...
    set_tcp_nodelay(fd);  // flush frames immediately, no Nagle buffering
    ...
    // Register socket for future send_event() pushes
    {
      std::lock_guard<std::mutex> lock(self->sse_mutex_);
      self->sse_clients_.push_back({fd});
    }

    // Spawn a lightweight ping task — does NOT hold any httpd worker thread
    struct PingCtx { IDFBackend *self; int fd; };
    auto *ctx = new PingCtx{self, fd};

    xTaskCreate(
      [](void *arg) {
        auto *ctx  = static_cast<PingCtx *>(arg);
        IDFBackend *self = ctx->self;
        int fd = ctx->fd;
        delete ctx;

        while (true) {
          vTaskDelay(pdMS_TO_TICKS(15000));  // 15 s heartbeat

          const char *ping = ": ping\n\n";
          int ret = send(fd, ping, strlen(ping), MSG_DONTWAIT);

          if (ret < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
            // Client gone — remove and close
            {
              std::lock_guard<std::mutex> lock(self->sse_mutex_);
              self->sse_clients_.remove_if(
    ...
    // Return without sending the NULL terminating chunk — that would close
    // the connection. The fd stays open; the ping task owns it from here.
    return ESP_OK;
  }
  ...
  // ── Entity command handler (wildcard /api/*) ───────────────────────────────
  static esp_err_t h_entity_cmd(httpd_req_t *req) {
    ...
    if (uri.find("/api/switch/") == 0) {
      ...
      for (auto *sw : App.get_switches()) {
        if (WebServerCustom::make_id(sw->get_name()) == id) {
          sw->toggle(); send_ok(); return ESP_OK;
        }
      }
#endif
    ...
    if (uri.find("/api/light/") == 0) {
      ...
      for (auto *light : App.get_lights()) {
        if (WebServerCustom::make_id(light->get_name()) == id) {
          ...
          std::string r = get_post_param(body, "r"),
                      g = get_post_param(body, "g"),
                      b = get_post_param(body, "b");
    ...
    if (uri.find("/api/button/") == 0) {
      ...
      for (auto *button : App.get_buttons()) {
        if (WebServerCustom::make_id(button->get_name()) == id) {
          button->press(); send_ok(); return ESP_OK;
        }
      }
#endif
      send_404(); return ESP_OK;
  ...
  void register_routes() {
    auto reg = [&](const char *uri, httpd_method_t method,
                   esp_err_t (*handler)(httpd_req_t *), void *ctx = nullptr) {
      httpd_uri_t h = {};
      h.uri      = uri;
      h.method   = method;
      h.handler  = handler;
